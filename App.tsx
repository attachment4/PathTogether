...
// Локализация
const tr = (lang: string, ru: string, en: string, uk?: string, be?: string, kk?: string): string => {
  if (lang === 'en') return en;
  if (lang === 'uk') return uk || ru;
  if (lang === 'be') return be || ru;
      ...
      {NAV.map(item => {
        ...
        return (
          <TouchableOpacity key={item.s} onPress={()=>onPress(item.s as Screen)}
            ...
            <Text style={{fontSize:10,fontWeight:isA?'700':'500',color:isA?tk.text:tk.text2}}>{lang==='en'?item.en:lang==='uk'?item.uk:lang==='be'?item.be:lang==='kk'?item.kk:item.ru}</Text>
          </TouchableOpacity>
        );
      })}
      </View>
  ...
  return (
    <View style={{ height: itemH * VISIBLE, overflow: 'hidden', width: 80, backgroundColor: tk.bg2 }}
      ...
      <View pointerEvents="none" style={{
        ...
        borderColor: tk.bg === '#0a0a0a' ? 'rgba(0,0,0,0.25)' : 'rgba(0,0,0,0.13)',
      }}/>
      <View pointerEvents="none" style={{
        ...
        <View style={{
          ...
          <View style={{alignItems:'center', paddingTop:12, paddingBottom:4}}>
            <View style={{width:36, height:3, borderRadius:2,
              backgroundColor: tk.bg === '#0a0a0a' ? 'rgba(0,0,0,0.10)' : 'rgba(0,0,0,0.12)'}}/>
          </View>
          ...
          <View style={{flexDirection:'row', justifyContent:'space-between',
            ...
            <TouchableOpacity onPress={clear}
              ...
              style={{width:36,height:36,borderRadius:18,
                backgroundColor: tk.bg === '#0a0a0a' ? 'rgba(0,0,0,0.07)' : 'rgba(0,0,0,0.05)',
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      if (!name) Storage.saveName(displayName).catch(()=>{}); // не ждём
      ...
      <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={{flex:1}}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:20,paddingBottom:60}} keyboardShouldPersistTaps="handled">
          <View style={{flexDirection:'row',alignItems:'center',gap:12,marginBottom:28}}>
            ...
            <Text style={{fontSize:20,fontWeight:'700',color:tk.text,flex:1}}>
          ...
          <Text style={{fontSize:9,color:tk.text3,letterSpacing:1.5,textTransform:'uppercase',marginBottom:10}}>{isEn?'Days':'Дни'}</Text>
          ...
          <TouchableOpacity onPress={submitHabit} activeOpacity={0.8}
            ...
            <Text style={{color:tk.bg,fontSize:14,fontWeight:'700'}}>
  ...
  if (screen==='invite') return (
    <View style={{flex:1,paddingTop:TOP,backgroundColor:tk.bg}}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:20,paddingBottom:60}}>
        ...
        <Text style={{fontSize:9,color:tk.text3,letterSpacing:1.5,textTransform:'uppercase',marginBottom:8}}>{isEn?'Or link':'Или ссылка'}</Text>
      ...
      <View style={{flex:1,paddingTop:TOP,backgroundColor:tk.bg}}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:20,paddingBottom:60}}>
          <View style={{flexDirection:'row',alignItems:'center',gap:12,marginBottom:24}}>
            ...
            <Text style={{fontSize:20,fontWeight:'700',color:tk.text,flex:1}}>{isEn?'Details':'Подробности'}</Text>
          ...
          <View style={{backgroundColor:tk.bg2,borderWidth:1,borderColor:tk.border,borderRadius:16,padding:20,alignItems:'center',gap:10,marginBottom:20}}>
            ...
            <Text style={{fontSize:20,fontWeight:'700',color:tk.text}}>{h.name}</Text>
          ...
          <Text style={{fontSize:9,color:tk.text3,letterSpacing:1.5,textTransform:'uppercase',marginBottom:10}}>{isEn?'Last 30 days':'Последние 30 дней'}</Text>
          ...
          <Text style={{fontSize:9,color:tk.text3,letterSpacing:1.5,textTransform:'uppercase',marginBottom:12}}>7 {isEn?'days':'дней'}</Text>
          {members.map(m=>(
