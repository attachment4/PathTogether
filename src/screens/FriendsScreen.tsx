...
import { View, Platform, Text, ScrollView, TouchableOpacity, Alert, Share } from 'react-native';
    ...
    <View style={{ flex: 1, backgroundColor: tk.bg, paddingTop: Platform.OS === 'ios' ? 50 : (require('react-native').StatusBar.currentHeight || 24) + 4 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}
        ...
        <View style={{ backgroundColor: tk.bg2, borderWidth: 1, borderColor: tk.border, borderRadius: 14, padding: 16 }}>
          <Text style={{ fontSize: 12, fontWeight: '500', color: tk.text, marginBottom: 6, letterSpacing: -0.2, fontSize: 16 }}>
          ...
          {invLink ? (
            <View style={{ gap: 8 }}>
              <View style={{ backgroundColor: tk.bg3, borderRadius: 10, borderWidth: 1,
                borderColor: tk.border, padding: 10 }}>
                <Text style={{ fontSize: 11, color: tk.text3, numberOfLines: 1 }}
                  numberOfLines={1}>{invLink}</Text>
              </View>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity onPress={() => Share.share({ message: invLink })}
                  style={{ flex: 1, backgroundColor: tk.text, borderRadius: 12,
                    padding: 12, alignItems: 'center' }}>
                  <Text style={{ color: tk.bg, fontSize: 13, fontWeight: '700' }}>
                    {(lang === 'en' ? 'Share link' : lang==='uk' ? 'Поділитися' : lang==='be' ? 'Падзяліцца' : lang==='kk' ? 'Бөлісу' : 'Поделиться')}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={onCopyLink}
                  ...
                  <Text style={{ color: tk.text2, fontSize: 13, fontWeight: '600' }}>
                    {(lang === 'en' ? 'Copy' : lang==='uk' ? 'Скопіювати' : lang==='be' ? 'Скапіяваць' : lang==='kk' ? 'Көшіру' : 'Скопировать')}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
